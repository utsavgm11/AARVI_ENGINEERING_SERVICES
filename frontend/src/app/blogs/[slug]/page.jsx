...
export default function BlogDetail() {
  ...
  // 💡 Safe URL Helpers for Image and Video
  const resolveMedia = (path) => {
    if (!path) return null;
    return path.startsWith('http') ? path : `${API_BASE}${path}`;
  };

  const coverImg  = resolveMedia(blog.cover_img);
  const videoUrl  = resolveMedia(blog.video_url); // properly handles local MP4 vs YouTube
                  ...
                  {relatedBlogs[0] && (
                    ...
                    >
                      {relatedBlogs[0].cover_img && (
                        <Image
                          src={resolveMedia(relatedBlogs[0].cover_img)}
                  ...
                  {relatedBlogs[1] && (
                    ...
                    >
                      {relatedBlogs[1].cover_img && (
                        <Image
                          src={resolveMedia(relatedBlogs[1].cover_img)}
